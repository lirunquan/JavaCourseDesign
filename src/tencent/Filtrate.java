...
public class Filtrate {
	public static void filtrate(String date) throws FileNotFoundException {
		...
		JSONArray data = JsonToData.testComplexJSONStrToJSONObject(COMPLEX_JSON_STR);
		...
		for (int i = 0; i < size; i++) {
			...
			if (jsonObject.getString("column").indexOf("军事") >= 0 || jsonObject.getString("column").indexOf("图片") >= 0
...
//				System.out.println("=============================================================");
				...
				while (input.hasNext()) {
					...
					if (jsonObject.getString("title").indexOf(word) != -1) {
						...
						if(mysqlOp.isDataExists("crawler ", "SELECT url FROM datas ", jsonObject.getString("url"), "url")) {
